...
class NmeaGpsNode(Node):
    def __init__(self):
        ...
        self.speed_pub = self.create_publisher(Float32, '/ground_speed_mps', qos_profile_sensor_data)
        self.track_pub = self.create_publisher(Float32, '/track_true_deg', qos_profile_sensor_data)
        self.nmea_pub = self.create_publisher(String, '/nmea_sentence', 10)
        # 新增 heading 角度发布器
        self.heading_pub = self.create_publisher(Float32, '/heading_deg', 10)
