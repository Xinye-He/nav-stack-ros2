...
public:
  BaleEstimatorNode() : Node("bale_pose_estimator") {
    ...
    // 新增：阴影检测参数
    this->declare_parameter<double>("shadow_delta_deg", 10.0);        // 扇区半角 (度)
    this->declare_parameter<double>("shadow_margin_front", 0.5);      // 草垛后多少米开始看
    this->declare_parameter<double>("shadow_length", 4.0);            // 阴影长度
    this->declare_parameter<int>("shadow_min_ground_points", 5);      // 阈值：地面点数
    this->declare_parameter<double>("shadow_ground_z_min", -0.3);     // 地面高度窗口
    this->declare_parameter<double>("shadow_ground_z_max",  0.3);
    ...
    // 阴影检测参数
    shadow_delta_deg_       = this->get_parameter("shadow_delta_deg").as_double();
    shadow_margin_front_    = this->get_parameter("shadow_margin_front").as_double();
    shadow_length_          = this->get_parameter("shadow_length").as_double();
    shadow_min_ground_pts_  = this->get_parameter("shadow_min_ground_points").as_int();
    shadow_ground_z_min_    = this->get_parameter("shadow_ground_z_min").as_double();
    shadow_ground_z_max_    = this->get_parameter("shadow_ground_z_max").as_double();
    ...
    // 新增：ROI 可视化 Marker 发布器
    roi_marker_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/bale_roi_markers", 1);
    RCLCPP_INFO(this->get_logger(), "DEBUG: roi_marker_pub_ created on /bale_roi_markers");
...
private:
  ...
  // 阴影检测（使用含地面的 ROI 点云）
  bool checkShadowForCluster(const Eigen::Vector3f& center,
                           const CloudT::Ptr& roi_cloud);

  // 在 RViz 中可视化 ROI 盒子（声明）
  void publishRoiMarker(const std::string& frame_id, const rclcpp::Time& stamp);
  ...
  void cloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    // 在 target_frame 下查询 rslidar 的位置（用于以雷达为原点做 ROI）
    double lidar_x = 0.0, lidar_y = 0.0, lidar_z = 0.0;
    try {
      // target_frame_ 通常是 "base_link"，第二个参数是雷达的 frame 名
      auto tf_bl_lidar = tf_buffer_->lookupTransform(
        target_frame_,                      // parent: base_link
        "rslidar",                          // child:  rslidar（根据你实际的 frame 名）
        msg_tf.header.stamp,
        rclcpp::Duration::from_seconds(0.05));

      lidar_x = tf_bl_lidar.transform.translation.x;
      lidar_y = tf_bl_lidar.transform.translation.y;
      lidar_z = tf_bl_lidar.transform.translation.z;
    } catch (const std::exception &e) {
      RCLCPP_WARN_THROTTLE(this->get_logger(), *clk, 2000,
        "lookupTransform(%s, rslidar) failed: %s",
        target_frame_.c_str(), e.what());
      // TF 拿不到雷达位姿，就按原来以 target_frame 原点做 ROI，避免直接退出
      lidar_x = 0.0;
      lidar_y = 0.0;
      lidar_z = 0.0;
    }

    // 每帧发布一次 ROI 盒子（在 target_frame 下）
    publishRoiMarker(msg_tf.header.frame_id, rclcpp::Time(msg_tf.header.stamp));
    ...
    for (const auto& p : cloud->points) {
      // 这里用的是“相对雷达”的坐标
      const float dx = p.x - static_cast<float>(lidar_x);
      const float dy = p.y - static_cast<float>(lidar_y);

      const float r = std::hypot(dx, dy);  // 相对雷达的水平距离
      const float z = p.z;           
    ...
    for (const auto& c : clusters) {
      ...
      const double D = 0.5 * (W + H);

      // === 新增 1：最长边是否近似“水平”？ ===
      {
        // base_link 下的“地面法线”（假定 z 轴竖直）
        const Eigen::Vector3f ground_n(0.0f, 0.0f, 1.0f);

        // 最长边方向（OBB 的第一主轴）
        Eigen::Vector3f long_dir = sorted_axes.col(0);
        if (!long_dir.allFinite()) continue;
        long_dir.normalize();

        // 计算与地面法线的夹角余弦的绝对值 |cosθ|
        const double cos_theta = std::fabs(long_dir.dot(ground_n));

        // 允许最长边与水平面最多倾斜 max_tilt_deg_ 度（可通过参数修改）
        const double max_tilt_clamped =
            std::max(0.0, std::min(89.0, max_tilt_deg_));  // 简单夹一下，避免超范围
        const double cos_max_tilt =
            std::cos(max_tilt_clamped * M_PI / 180.0);

        // 如果 |cosθ| 很大，说明最长边几乎竖直（与 z 轴对齐）→ 认为是“立着的东西”(人、杆子等)，丢弃
        if (cos_theta > cos_max_tilt) {
          // RCLCPP_DEBUG(this->get_logger(), "Reject cluster: longest axis too vertical (|cosθ|=%.2f)", cos_theta);
          continue;
        }
      }
      // === 新增 1 结束 ===
      ...
      // === 新增 2：阴影检测（使用含地面的 roi 点云） ===
      if (!checkShadowForCluster(center, roi)) {
        // 该簇后方没有明显“地面稀疏区”，不认为是草捆
        continue;
      }
      // === 新增 2 结束 ===
  ...
  // 新增：阴影检测参数
  double shadow_delta_deg_;
  double shadow_margin_front_;
  double shadow_length_;
  int    shadow_min_ground_pts_;
  double shadow_ground_z_min_;
  double shadow_ground_z_max_;
...
void BaleEstimatorNode::publishRoiMarker(const std::string& frame_id,
                                         const rclcpp::Time& stamp) {
  if (!roi_marker_pub_) {
    RCLCPP_WARN(this->get_logger(), "ROI marker publisher not initialized");
    return;
  }
  ...
  // 查询雷达在 frame_id（通常是 base_link）下的位置
  double lidar_x = 0.0, lidar_y = 0.0, lidar_z = 0.0;
  try {
    auto tf_bl_lidar = tf_buffer_->lookupTransform(
      frame_id,                        // parent
      "rslidar",                       // child（根据你的实际 frame 名）
      stamp,
      rclcpp::Duration::from_seconds(0.05));
    lidar_x = tf_bl_lidar.transform.translation.x;
    lidar_y = tf_bl_lidar.transform.translation.y;
    ...
    RCLCPP_WARN(this->get_logger(), "publishRoiMarker: lookupTransform(%s, rslidar) failed: %s",
  ...
  const float RMAX = static_cast<float>(r_max_);
  const float ZMIN = static_cast<float>(z_min_);
  const float ZMAX = static_cast<float>(z_max_);

  // 中心 = 雷达位置 + 垂直方向的中点
  box.pose.position.x = static_cast<float>(lidar_x);
  box.pose.position.y = static_cast<float>(lidar_y);
  box.pose.position.z = (ZMIN + ZMAX) * 0.5f;

  box.pose.orientation.x = 0.0f;
  box.pose.orientation.y = 0.0f;
  box.pose.orientation.z = 0.0f;
  box.pose.orientation.w = 1.0f;

  box.scale.x = std::max(0.1f, RMAX * 2.0f);
  box.scale.y = std::max(0.1f, RMAX * 2.0f);
  box.scale.z = std::max(0.1f, (ZMAX - ZMIN));

  box.color.r = 1.0f;
  box.color.g = 0.0f;
  box.color.b = 0.0f;
  box.color.a = 0.4f;  // 半透明红色

  box.lifetime = rclcpp::Duration(0, 0);

  ma.markers.push_back(box);
  roi_marker_pub_->publish(ma);
}

bool BaleEstimatorNode::checkShadowForCluster(const Eigen::Vector3f& center,
                                              const CloudT::Ptr& roi_cloud)
{
  ...
  // 簇中心在 base_link 下的位置（base_link 原点 ~= 车体）
  ...
  const double ctheta = std::atan2(cy, cx);

  // 参数准备
  const double dtheta_rad = shadow_delta_deg_ * M_PI / 180.0;
  const double margin_front = shadow_margin_front_;
  const double shadow_len   = shadow_length_;
  const double r_start      = cr + margin_front;
  const double r_end        = cr + shadow_len;

  const double zmin = shadow_ground_z_min_;
  const double zmax = shadow_ground_z_max_;

  int ground_like_points = 0;

  for (const auto& p : roi_cloud->points) {
    const double r  = std::hypot(p.x, p.y);
    if (r < r_start || r > r_end) continue;

    const double th = std::atan2(p.y, p.x);
    double dth = std::atan2(std::sin(th - ctheta), std::cos(th - ctheta));  // [-pi,pi]
    dth = std::fabs(dth);
    if (dth > dtheta_rad) continue;
  ...
  // 如果这段扇形 + 半径范围内几乎没有“接近地面”的点，就认为有阴影
  return (ground_like_points < shadow_min_ground_pts_);
}
