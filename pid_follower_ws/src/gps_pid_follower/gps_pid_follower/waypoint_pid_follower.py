...
class WaypointPIDFollower(Node):
    ...
    def __init__(self):
        ...
        # PRE_STOP：先保持 0 速 0 角一段时间，让车速降到接近 0，再进入真正 SPIN
        self.declare_parameter('spin_pre_stop_time_s', 0.5)  # 预停止时间，默认 0.5s
        self.spin_pre_stop_time_s = float(self.get_parameter('spin_pre_stop_time_s').value)

        self.spin_state = 'IDLE'
        self.spin_pre_stop_until = 0.0  # PRE_STOP 结束时间戳（秒）
    ...
    def decide_vcu_action(self,
        ...
        if self.spin_state == 'PRE_STOP':
            ...
            if now_sec < self.spin_pre_stop_until:
                ...
                # 预停止时间到，切换为真正 SPIN 状态
                self.spin_state = 'SPIN'
                # 不 return，继续往下执行 SPIN 逻辑
        ...
        if self.spin_state == 'IDLE' and (want_spin_corner or want_spin_emergency):
            ...
            if already_stop:
                ...
                # 从低速/高速切换到原地 SPIN:
                # 先进入PRE_STOP，保持一段时间0速0角，让车速降到接近0
