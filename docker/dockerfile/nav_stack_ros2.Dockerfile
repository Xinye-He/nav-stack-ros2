...
RUN pip3 install websockets -i https://mirrors.aliyun.com/pypi/simple/
...
#Lidar_ws依赖
WORKDIR $ROS_WS/src
RUN git clone -b humble https://github.com/ros-perception/perception_pcl.git
RUN git clone -b humble https://github.com/ros2/geometry2.git
RUN git clone -b humble https://github.com/ros2/common_interfaces.git

WORKDIR $ROS_WS
RUN /bin/bash -c "source /opt/ros/${ROS_DISTRO}/install/setup.bash && \
    colcon build --packages-select geometry2 \
    --cmake-args -Wno-dev -DBUILD_TESTING=OFF -DCMAKE_PREFIX_PATH=/usr/local \
    --parallel-workers $(nproc)"
RUN /bin/bash -c "source /opt/ros/${ROS_DISTRO}/install/setup.bash && \
    ...
    --parallel-workers $(nproc)"
RUN /bin/bash -c "source /opt/ros/${ROS_DISTRO}/install/setup.bash && \
    colcon build --packages-up-to perception_pcl \
    --cmake-args -Wno-dev -DBUILD_TESTING=OFF -DCMAKE_PREFIX_PATH=/usr/local \
    --parallel-workers $(nproc)"
